...
def generate_income_table(item_list, id, user, date, save_Excle=True):
    ...
    ws.page_margins = PageMargins(left=0.75, right=0.75, top=1, bottom=1)  # 设置页边距(单位:英寸)
    # -------------开始设置格式-------------#
    # 设置缩放比例
    ...
    col_width = [13, 12, 8, 8, 4.2, 4.2, 4.2, 4.2, 4.2, 4.2, 4.2, 4.2, 4.2, 4.2, 4.2,
                 4.2, 4.2, 4.2, 5.6, 5.6, 4.7, 5.6, 5.6, 4.7]
    col = [chr(i) for i in range(ord("A"), ord("X") + 1)]
    ...
    # 设置边框 颜色 标题
    rangeCell = ws['A4:X{}'.format(6 + data_len)]
    ...
    title_cell_2 = ws['A2']
    title_cell_2.value = '{}年{}月利润表'.format(date[:4], date[5:7])
    title_cell_2.font = Font(size=12)
...
if __name__ == '__main__':
    generate_income_table(DATA, 1, '刘朝静', '2022-01')
